...
class WishlistController extends UsersDeckController
{
    ...
    public function getList() {
        $authUser = Auth::user();
        if ($authUser) {
            return redirect()->action('WishlistController@getUserList', $authUser->id);
        }
    }
    ...
    public function getUserList(User $user) {
        ...
        if ($authUser && $authUser->id == $user->id) {
            $items = UsersDeck::with('user', 'deck')
                ...
                ->where('in_wishlist', '>', 0)
                ->orderBy('weight_wishlist', 'DESC')->orderBy('created_at', 'DESC')
                ->take(20)->get();
            return view('collections.wishlist', [
        ...
        $items = UsersDeck::with('user', 'deck')
            ...
            ->take(20)->get();
        return view('collections.wishlist', [
            'page_title' => "The wishlist of " . $user->name . ' user.',
            'user' => $user,
    ...
    public function postAdd(Request $request) {
        ...
        return json_encode([
            ...
            'collection_count'=>$users_deck->in_wishlist,
            'deck_id'=>$deck->id
        ]);
    }
    ...
    public function postRemove(Request $request) {
        ...
        return json_encode([
            ...
            'collection_count'=>$users_deck ? $users_deck->in_wishlist : 0,
            'deck_id'=>$deck->id
        ]);
    }
    ...
    public function postSearch(User $user, Request $request) {
        ...
        return view($this->resolveViewModeTpl($viewMode), ['user' => $user, 'items' => $items, 'type'=>'wishlist'])->render();
    }

    public function postReorder(Request $request) {
        $new_weights = $this->updateCollectionOrder('wishlist', $request);
